...
const sessionHandler = require('./lib/sessionHandler'); // Import the session handler
...
class WhatsAppBot {
  ...
  async connect() {
    ...
    this.conn.ev.on('creds.update', async (update) => {
      await saveCreds(update); 
      await sessionHandler.saveCreds(update); 
    });
  ...
  async handleConnectionUpdate(s) {
    ...
    if (connection === 'connecting') {
      ...
      const str = \\`\xstro connected\nversion: ${packageVersion}\ncommands: ${totalPlugins}\nmode: ${workType}\\\`;
      await this.conn.sendMessage(this.conn.user.id, { text: str });
      await this.conn.sendMessage(this.conn.user.id, { text: 'Syncing Chat Please Be Patient' });
  ...
  async handleCalls(call) {
    ...
    if (config.ANTI_CALL && status === 'offer') {
      if (config.ANTI_CALL === 'true') {
        ...
        await this.conn.sendMessage(from, { text: 'Calls not allowed.' });
      } else if (config.ANTI_CALL === 'block') {
        await this.conn.rejectCall(callId, from);
        await this.conn.sendMessage(from, { text: 'Calls are not allowed, You have been blocked.' });
